...
@interface SnapKRequest : NSMutableURLRequest
...
/** @brief Replaces header field values. Useful for changing the user-agent on the fly.
 @discussion Pass \c nil to remove previous overrides. [SnapKRequest overrideHeaderValues:forEndpoint:] takes precedence over this method and affects every request, but otherwise functions the same.
...
/** @brief Replaces header field values for a specific endpoint.
 @discussion Pass \c nil to \c headers to remove previous overrides for a given endpoint. See [SnapKRequest overrideHeaderValues:] for more information.
 This method takes precedence over [SnapKRequest overrideHeaderValuesGlobally:] and is used before [SnapKRequest overrideEndpoints:], so make sure to override the original endpoint.
 @param headers A dictionary of header-value key-value pairs, i.e. @code @{@"User-Agent": @"iOS 2.0"} @endcode
...
/** @brief Replaces values for specific query keys in the scope of the given endpoint.
 ...
 This method takes precedence over [SnapKRequest overrideValuesForKeysGlobally:] and is used before [SnapKRequest overrideEndpoints:], so make sure to override the original endpoint.
 ...
 having called [SnapKRequest overrideEndpoints:] with this dictionary: @code
 ...
 If you \a did want to override that value in \a every request, you would pass @code @{@"action": newValue} @endcode to [SnapKRequest overrideValuesForKeysGlobally:].
 */
...
/** @brief Replaces values for specific query keys in every request.
 @discussion Pass \c nil to remove previous overrides. [SnapKRequest overrideValuesForKeys:forEndpoint:] takes precedence over this method and affects every request, but otherwise functions the same. */
...
/** @brief Replaces endpoint \c some_endpoint with \e endpoints[some_endpoint].
 @discussion [SnapKRequest overrideValuesForKeys:forEndpoint:] and [SnapKRequest overrideHeaderValues:forEndpoint:] take precedence over this method.
