...
- (void)postStory:(SKBlob *)blob for2:(NSTimeInterval)duration completion:(ErrorBlock)completion {
...
- (void)uploadStory:(SKBlob *)blob completion:(ResponseBlock)completion {
    ...
    [SnapKRequest postTo:SKEPStories.upload query:query headers:headers token:self.authToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
...
- (void)loadStoryBlob:(SKStory *)story completion:(ResponseBlock)completion {
    ...
    if (story.needsAuth) {
        ...
        [SnapKRequest postTo:url query:query gauth:self.googleAuthToken token:self.authToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
            if (!error) {
