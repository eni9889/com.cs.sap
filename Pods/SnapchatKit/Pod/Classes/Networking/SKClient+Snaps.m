...
- (void)loadSnapWithIdentifier:(NSString *)identifier completion:(ResponseBlock)completion {
    ...
    [SnapKRequest postTo:SKEPSnaps.loadBlob query:query gauth:self.googleAuthToken token:self.authToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
        // Did get snap
        if ([(NSHTTPURLResponse *)response statusCode] == 200) {
            ...
            if ([data isJPEG] || [data isMPEG4]) {
                ...
                if (blob)
                    ...
                    completion(nil, [SnapKRequest errorWithMessage:@"Error initializing blob with data" code:1]);
            ...
            // Needs to be unzipped
                ...
                [SSZipArchive unzipFileAtPath:path toDestination:unzip completion:^(NSString *path, BOOL succeeded, NSError *error) {
                    if (succeeded) {
                        ...
                        if (blob)
                            ...
                            completion(nil, [SnapKRequest errorWithMessage:@"Error initializing blob" code:2]);
                    } else {
                        SKLog(@"%@", error);
                    }
                }];
                ...
                SKBlob *blob = [SKBlob blobWithData:data];
                if (blob)
                    completion(blob, [SnapKRequest errorWithMessage:@"Unknown blob format" code:[(NSHTTPURLResponse *)response statusCode]]);
                else
                    completion(nil, [SnapKRequest errorWithMessage:@"Error initializing blob with data" code:1]);
            } else {
                completion(nil, [SnapKRequest errorWithMessage:[NSString stringWithFormat:@"Error retrieving snap: %@", identifier] code:[(NSHTTPURLResponse *)response statusCode]]);
            }
        // Failed to get snap
        } else {
