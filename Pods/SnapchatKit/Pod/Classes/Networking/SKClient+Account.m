...
- (void)handleCallback:(NSDictionary *)json error:(NSError *)error callback:(ErrorBlock)completion {
    if (!error && [json[@"logged"] boolValue]) {
        ...
        completion(error ?: [SnapKRequest errorWithMessage:json.description code:1]);
    }
}
...
- (void)downloadSnaptag:(ResponseBlock)completion {
    ...
    NSDictionary *query = @{@"image": self.currentSession.QRPath,
                            @"type": @"SVG",
                            @"username": self.username};
    [SnapKRequest postTo:SKEPAccount.snaptag query:query gauth:self.googleAuthToken token:self.authToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
            ...
            if (!error) {
                if ([(NSHTTPURLResponse *)response statusCode] == 200) {
                    if (data.length)
                        ...
                        completion(nil, [SnapKRequest errorWithMessage:@"Error retrieving snaptag" code:1]);
                }
            } else {
...
- (void)downloadAvatar:(NSString *)username completion:(ResponseBlock)completion {
    ...
    [SnapKRequest postTo:SKEPAccount.avatar.get query:query gauth:self.googleAuthToken token:self.authToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (!error) {
                if ([(NSHTTPURLResponse *)response statusCode] == 200) {
                    if (data.length)
