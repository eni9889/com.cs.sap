    ...
    if (error) {
        ...
        if (jsonError) {
            ...
            if ([html containsString:@"<html><head>"]) {
                // Invalid request
                callback(nil, [SnapKRequest errorWithMessage:html.textFromHTML code:code], response);
            } else {
                // ???
            ...
            if (code > 199 && code < 300) {
                ...
                if (logged) {
                    if (logged.integerValue == 1)
                        ...
                        callback(nil, [SnapKRequest errorWithMessage:json[@"message"] code:[json[@"status"] integerValue]], response);
                } else {
                    callback(json, nil, response);
                }
            } else {
                // Failed with a message
                error = [SnapKRequest errorWithMessage:json[@"message"] code:code];
                callback(nil, error, response);
            }
        }
        else {
            callback(nil, [SnapKRequest unknownError], response);
        }
...
- (void)postTo:(NSString *)endpoint query:(NSDictionary *)query callback:(ResponseBlock)callback {
    ...
    [SnapKRequest postTo:endpoint query:query gauth:self.googleAuthToken token:self.authToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
...
- (void)get:(NSString *)endpoint callback:(ResponseBlock)callback {
    ...
    [SnapKRequest get:endpoint callback:^(NSData *data, NSURLResponse *response, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (!error) {
                NSInteger code = [(NSHTTPURLResponse *)response statusCode];
                if (code == 200)
                    ...
                    if ([html containsString:@"<html><head>"]) {
                        // Invalid request
                        callback(nil, [SnapKRequest errorWithMessage:html.textFromHTML code:code]);
                    } else {
                        callback(nil, [SnapKRequest errorWithMessage:@"Unknown error" code:[(NSHTTPURLResponse *)response statusCode]]);
                    }
                }
            } else {
                callback(nil, error);
...
- (void)getAuthTokenForGmail:(NSString *)gmailAddress password:(NSString *)password callback:(StringBlock)callback {
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (error) {
            ...
            callback(nil, [SnapKRequest unknownError]);
        }
...
- (void)getDeviceToken:(DictionaryBlock)completion {
    ...
    if (dt1i && dt1v)
        ...
        [SnapKRequest postTo:SKEPDevice.identifier query:@{} headers:@{SKHeaders.clientAuthToken: @"Bearer "} token:nil callback:^(NSData *data, NSURLResponse *response, NSError *error) {
            if (error) {
                completion(nil, error);
            } else if (data) {
                NSError *jsonError;
                ...
                if (jsonError ) {
                    ...
                    completion(nil, [SnapKRequest unknownError]);
                }
            } else {
                completion(nil, [SnapKRequest unknownError]);
            }
    ...
    [[session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (!error) {
            ...
            if ([json[@"code"] integerValue] == 200)
                ...
                callback(nil, [SnapKRequest errorWithMessage:json[@"message"] code:[json[@"status"] integerValue]]);
        } else {
            callback(nil, error);
        }
    }] resume];
                            ...
                            else
                            {
                                ...
                                [self getClientAuthToken:username password:password timestamp:timestamp callback:^(NSString *clientAuthToken, NSError *error5) {
                                    if (error5 || !clientAuthToken) {
                                        completion(nil, error5 ?: [SnapKRequest errorWithMessage:@"Could not retrieve client auth token." code:1]);
                                    } else {
...
- (void)signInWithData:(NSDictionary *)loginData username:(NSString *)username password:(NSString *)password completion:(DictionaryBlock)completion {
    ...
    SnapKRequest *request    = [[SnapKRequest alloc] initWithPOSTEndpoint:SKEPAccount.login token:nil query:post headers:headers ts:timestamp];
    NSURLSession *session = [NSURLSession sessionWithConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]];
...
- (void)signOut:(ErrorBlock)completion {
    [SnapKRequest postTo:SKEPAccount.login query:@{@"username": self.currentSession.username} gauth:self.googleAuthToken token:self.authToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (kVerboseLog) {
            ...
            if (result.length == 0) {
                ...
                completion([SnapKRequest errorWithMessage:result code:1]);
            }
        }
    }];
}
    ...
    [self postTo:SKEPAccount.registration.start query:query callback:^(NSDictionary *json, NSError *error) {
        if (!error) {
            ...
            // Failed for some reason
                ...
                completion(nil, [SnapKRequest errorWithMessage:json[@"message"] code:[json[@"status"] integerValue]]);
            }
        ...
        [self postTo:SKEPAccount.registration.username query:query callback:^(NSDictionary *json, NSError *error) {
            if (!error) {
                ...
                if (kDebugJSON && !self.currentSession) {
                    completion([SnapKRequest unknownError]);
            ...
            // Failed for some reason
                ...
                completion([SnapKRequest errorWithMessage:json[@"message"] code:[json[@"status"] integerValue]]);
            }
        }];
    }];
}
...
- (void)getCaptcha:(ArrayBlock)completion {
    [SnapKRequest postTo:SKEPAccount.registration.getCaptcha query:@{@"username": self.username} gauth:self.googleAuthToken token:self.authToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
        // Did get captcha ZIP
        if ([(NSHTTPURLResponse *)response statusCode] == 200) {
            ...
            [SSZipArchive unzipFileAtPath:dataPath toDestination:imagesPath completion:^(NSString *path, BOOL succeeded, NSError *error) {
                ...
                // Error unzipping
                    ...
                    completion(nil, [SnapKRequest errorWithMessage:@"Error unzipping captcha" code:1]);
                }
            }];
        // Failed to get captcha ZIP
        } else {
            completion(nil, [SnapKRequest errorWithMessage:@"Unknown error" code:[(NSHTTPURLResponse *)response statusCode]]);
        }
    }];
}
...
- (void)sendPhoneVerification:(NSString *)mobile sendText:(BOOL)sms completion:(DictionaryBlock)completion {
    ...
    if (digits.count != 10 && digits.count != 11) {
        completion(nil, [SnapKRequest errorWithMessage:@"Invalid phone number" code:400]);
    }
...
- (void)verifyPhoneNumberWithCode:(NSString *)code completion:(ErrorBlock)completion {
    ...
    [SnapKRequest postTo:SKEPAccount.registration.verifyPhone query:query gauth:self.googleAuthToken token:SKConsts.staticToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
...
- (void)sendEvents:(NSArray *)events data:(NSDictionary *)snapInfo completion:(ErrorBlock)completion {
    ...
    [SnapKRequest postTo:SKEPUpdate.snaps query:query gauth:self.googleAuthToken token:self.authToken callback:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (completion)
            dispatch_async(dispatch_get_main_queue(), ^{
